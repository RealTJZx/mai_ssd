...
if __name__ == "__main__":
    ...
    time_today_ymd = datetime.datetime.now()
    time_today_ymd = time_today_ymd.strftime("_%Y%m%d_%H_%M_")
    # zx
    select_dataset = {
        "NEU": 1,
        "LWD": 2,
    ...
    # select_key = input('请选择数据集（NEU PCB NEU_S300）：')
    select_key = "pcb"
    select_key_str = select_key.upper()
    select_key = select_dataset.get(select_key_str)
    print(f'数据集索引号:{select_key}，为{select_key_str}，类型为{type(select_key)}')
    ...
    #---------------------------------------------------------------------#
    # zx
    # NEU: 'model_data/voc_neu_classes.txt'
    if select_key == 1:
    ...
    if select_key == 1:
        ...
        raise ValueError("不存在这个数据集对应的输入大小")
    #------------------------------------------------------#
    #   vgg或者mobilenetv2或者resnet50
    #------------------------------------------------------#
    # zx
    #   "cspz", "cspz_fpn", "cspz_attention", "cspz_fpn_attention"
    ...
    #------------------------------------------------------#
    # zx - project_default - [30, 60, 111, 162, 213, 264, 315]
    ...
    # zx
    if select_key == 1:
        train_annotation_path = '2007_train_NEU.txt'
        val_annotation_path = '2007_val_NEU.txt'
    elif select_key == 2:
        train_annotation_path = '2007_train_LWD.txt'
        val_annotation_path = '2007_val_LWD.txt'
    elif select_key == 3:
        train_annotation_path = '2007_train_PCB.txt'
        val_annotation_path = '2007_val_PCB.txt'
    else:
        raise ValueError("没有这个数据集的划分数据！！！")
    ...
    if True:
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            # zx
            time_consuming = time.perf_counter() - start_time
            print("消耗时间:{}".format(time_consuming))
            file_name_path = "TR{rc_time}SSD_{rc_dataset}_{rc_backbone}_FE{rc_freeze}_SZ{rc_freeze_bs}_UNE{rc_uf}_SZ{rc_uf_bs}.txt".format(
                rc_time=time_today_ymd,
                rc_dataset=select_key_str,
                rc_backbone=backbone,
                rc_freeze=Freeze_Epoch if Freeze_Train else "From",
                rc_freeze_bs=Freeze_batch_size if Freeze_Train else "Zero",
                rc_uf=UnFreeze_Epoch - Freeze_Epoch if Freeze_Train else UnFreeze_Epoch,
                rc_uf_bs=Unfreeze_batch_size,
            )
            with open(file_name_path, mode='w') as time_file:
                time_file.write(str(time_consuming))
