...
from nets.zx.cspz import CSPDarknet as cspz
...
class SSD300(nn.Module):
    def __init__(self, num_classes, backbone_name, pretrained = False):
        ...
        self.default_net_list = ["vgg", "mobilenetv2"]
        self.better_net_list = ["cspz", "cspz_fpn", "cspz_attention", "cspz_fpn_attention"]
        ...
        # zx
        # self.better_net_list = ["cspz", "cspz_fpn", "cspz_attention", "cspz_fpn_attention"]
        elif backbone_name in self.better_net_list:
            if backbone_name == "cspz":
                self.darknet = cspz(dep_mul=0.33, wid_mul=0.5)
            elif backbone_name == "cspz_fpn":
                self.darknet = cspz(dep_mul=0.33, wid_mul=0.5, use_fpn=True)
            elif backbone_name == "cspz_attention":
                self.darknet = cspz(dep_mul=0.33, wid_mul=0.5, use_attention=True)
            elif backbone_name == "cspz_fpn_attention":
                self.darknet = cspz(dep_mul=0.33, wid_mul=0.5, use_fpn=True, use_attention=True)
            else:
                raise  ValueError("CSPDarknet 不存在这个类型！")
            self.base_channel = self.darknet.base_channels
            mbox = [6, 6, 6, 6, 6, 6]
            loc_layers = []
            conf_layers = []
            backbone_source = self.darknet.feature_channels[2:]
            for k, v in enumerate(backbone_source):
                loc_layers += [nn.Conv2d(v, mbox[k] * 4, kernel_size=3, padding=1)]
                conf_layers += [nn.Conv2d(v, mbox[k] * num_classes, kernel_size=3, padding=1)]
        else:
    ...
    def forward(self, x):
        ...
        if self.backbone_name in self.default_net_list:
            #---------------------------#
            #   获得conv4_3的内容
            #   shape为38,38,512
            #---------------------------#
        ...
        elif self.backbone_name in self.better_net_list:
            x_d8, x_d16, x_d32, xn_4, xn_5, xn_6 = self.darknet(x)
