...
if __name__ == "__main__":
    ...
    file_path = os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                             "../../../DATASETS/detection/PREDICT/NEU_selected"))
    # zx
    ssd = SSD(
        model_path="logs/TR_20240319_01_55_SSD_PCB_cspz_fpn_attention_FEFrom_SZZero_UNE300_SZ16/ep267-loss1.447-val_loss1.286.pth",
    ...
    #-------------------------------------------------------------------------#
    dir_origin_path = r"L:\BaiduSyncdisk\MasterWorkSpace\DATASETS\detection\PREDICT\Paper\PCB_for_paper"
    dir_save_path   = r"L:\BaiduSyncdisk\MasterWorkSpace\DATASETS\detection\PREDICT\Paper\RESULTS\massd"
    #-------------------------------------------------------------------------#
    ...
    if mode == "predict":
        ...
        imgs_path = glob.glob(file_path + '/*.jpg')
        total_time_list = []
        total_time = 0

        print('目前检测图片数量：', len(imgs_path))
        n_index = 0

        for i in imgs_path:
            img = Image.open(i)
            tact_time = ssd.get_FPS(img, test_interval)
            total_time_list.append(tact_time)
            n_index += 1
        ...
        avg_time = total_time / len(total_time_list)
